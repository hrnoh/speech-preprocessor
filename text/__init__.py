...
def _text_to_sequence(text, cleaner_names, as_token):
    ...
    if not isEn and hparams.use_phoneme:
        text = g2pk(text)

    if isEn and hparams.use_phoneme:
        clean_text = _clean_text(text, cleaner_names)
        phonemes = g2p(clean_text)
...
def _phonemes_to_sequence(symbols):
    clean_phone = []
    for s in symbols:
        if '@' + s in _symbol_to_id:
            ...
            clean_phone.append(s)
    return [_symbol_to_id[p] for p in clean_phone if _should_keep_symbol(p)]
...
if __name__=='__main__':
    ...
    #     sequence.extend([_symbol_to_id[c] for c in text])
